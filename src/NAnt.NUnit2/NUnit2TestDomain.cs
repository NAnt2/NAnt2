    ...
    internal class NUnit2TestDomain {
        ...
        public TestRunner CreateRunner(FileInfo assemblyFile, FileInfo configFile, StringCollection referenceAssemblies) {
            ...
            if (privateBinPath != null) {
                string [] privateBinPaths = privateBinPath.Split(Path.PathSeparator);
            ...
            // create test domain - returns null for .NET Standard / Core / .NET 5.0 or later
            _domain = CreateDomain(assemblyFile.Directory, assemblyFile, configFile);
#if NET40_OR_GREATER
...
#else
            Activator.CreateInstance(typeof(AssemblyResolveHandler),
                bindingAttr: BindingFlags.Public | BindingFlags.Instance,
                binder: null,
                args: new object[] { probePaths, references },
                CultureInfo.InvariantCulture);
#endif
            // create testrunner
...
#else
            // FIXME: Requires proper implementation for .NET Standard / Core / .NET 5.0 or later
            // We can either return this null or PlatformNotSupportedException
            return null;
#endif
        ...
        private RemoteTestRunner CreateTestRunner(AppDomain domain) {
            ...
            return (RemoteTestRunner) oh.Unwrap();
#else
            RemoteTestRunner runner = (RemoteTestRunner)Activator.CreateInstance(
